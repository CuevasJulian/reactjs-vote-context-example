...
import { ImageRouter } from '@utils';
import { RulingProvider, RulingContext } from '@store'
import { TestCode } from '@views'
import './style.css';

const ZemogaUI = () => {

    const [ isMobil, setMobil ] = useState(false);
    const rulingContext = React.useContext(RulingContext);
    const { Option } = Select;
    const [ typeView, setTypeView ] = useState('list');
    const { rulingItems, setRulingItems } = rulingContext;
    ...
    const resize = () => {
        if( (window.innerWidth < 500) != isMobil){
            ...
            setMobil(is);
            if( window.innerWidth < 500 )setTypeView('grid')
        } 
    }
        ...
        <>
            ...
            <div className="max-centered">
                ...
                <main role="main">
                    ...
                    <h2 style={{textAlign:'left'}}>Previous Rulings</h2>
                    { !isMobil && (
                        <Select 
                        value={typeView}
                        onChange={(e)=>setTypeView(e)}
                        className={'type-order-select'}>
                            <Option value={'list'}>List</Option>
                            <Option value={'grid'}>Grid</Option>
                        </Select>
                    )}
                    <Row gutter={[28,28]} className={'row-rulings'}>
                        
                        <RulingContext.Consumer>
                            {
                                rulings => {
                                    return rulings.rulingItems.data.map((e,i) => {
                                        return (
                                            <TestCode type={typeView} key={i} data={e} rawData={{data:rulingItems,setData:setRulingItems}}/>
                                        )
                                    });   
                                }
                            }
